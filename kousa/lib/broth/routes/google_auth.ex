defmodule Broth.Routes.GoogleAuth do
  ...
  get "/web" do
    ...
    |> Plug.Conn.put_private(:ueberauth_request_options, %{
      callback_url: Application.get_env(:kousa, :api_url) <> "/auth/google/callback",
      options: [
  ...
  def handle_callback(%Plug.Conn{assigns: %{ueberauth_failure: failure}} = conn) do
    Logger.warn("google oauth failure: #{inspect(failure)}")
  ...
  def handle_callback(
        %Plug.Conn{private: %{google_user: user, google_token: %{access_token: access_token}}} =
          conn
      ) do
    try do
      {_, db_user} = Users.google_find_or_create(user, access_token)
